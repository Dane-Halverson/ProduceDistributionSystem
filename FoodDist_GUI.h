...
public:
  FoodDist_GUI();
  virtual ~FoodDist_GUI();

  void assignFoodDistSystem( std::shared_ptr<FoodDist_DataSource> fdistPtr );
...
private:
  ...
  bool show_schoolList_window;

  std::shared_ptr<FoodDist_DataSource> m_foodDistSysPtr;
  
  int m_selectedSchoolIndex;
  std::string m_selectedSchoolName;
  float m_selectedSchoolBudget;

  bool m_receivedProduce;
  

  static constexpr std::string_view m_windowTitle{ "Food Distribution Coordinator" };
};
